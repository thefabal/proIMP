        ...
        private void btnDatabaseClean_Click( object sender, EventArgs e ) {
            ...
            dbCommand.CommandText = 
                (@"DELETE 
                    FROM image 
                   WHERE 
                    image_id IN(SELECT t1.image_id FROM image AS t1 LEFT JOIN product AS t2 ON t1.image_id = t2.product_imageid WHERE t2.product_id IS NULL)");
            try {
            ...
            dbCommand.CommandText = 
                (@"DELETE 
                    FROM stock_flow 
                   WHERE 
                    sflow_id IN(SELECT t1.sflow_id FROM stock_flow AS t1 LEFT JOIN stock AS t2 ON t1.sflow_sid = t2.stock_id WHERE t2.stock_id IS NULL)");
            try {
                numberOfRecords += dbCommand.ExecuteNonQuery();
            } catch {
