    ...
    public partial class frmStockFlowIn:Form {
        ...
        private void frmStockFlowIn_Load( object sender, EventArgs e ) {
            ...
            if( strStockFlowID == string.Empty ) {
                ...
                dbCommand.CommandText = "SELECT t1.stock_id, t1.stock_type, t1.stock_date, t1.stock_supplier, t2.supplier_name, t1.stock_desc FROM stock AS t1 LEFT JOIN supplier AS t2 ON t1.stock_supplier = t2.supplier_id WHERE t1.stock_id = '" + strStockFlowID + "'";
        ...
        private void btnProductAdd_Click( object sender, EventArgs e ) {
            ...
            try {
                ...
                if( strStockFlowID != string.Empty ) {
                    this.dbCommand.CommandText = "INSERT INTO stock_flow (sflow_id, sflow_sid, sflow_productid, sflow_warehouseid, sflow_quantity, sflow_price) VALUES(NULL, '" + strStockFlowID + "', '" + lvi.SubItems[ 0 ].Text + "', '" + lvi.SubItems[ 3 ].Text + "', '" + lvi.SubItems[ 5 ].Text + "', '" + lvi.SubItems[ 6 ].Text + "')";
        ...
        private void btnStockSave_Click( object sender, EventArgs e ) {
            if( cbStockSupplier.SelectedIndex != -1 ) {
                if( strStockFlowID == string.Empty ) {
                    ...
                    try {
                        dbCommand.CommandText = "INSERT INTO stock (stock_id, stock_type, stock_date, stock_supplier, stock_desc) VALUES(NULL, 1, '" + dtpStockDate.Value.ToString( "yyyy-MM-dd" ) + "', '" + ( (SupplierItem)cbStockSupplier.SelectedItem ).SupplierID + "', '" + tbStockDesc.Text.Replace( "'", "''" ) + "')";
                    ...
                    dbCommand.CommandText = "UPDATE stock SET stock_date = '" + dtpStockDate.Value.ToString( "yyyy-MM-dd" ) + "', stock_supplier = '" + ( (SupplierItem)cbStockSupplier.SelectedItem ).SupplierID + "', stock_desc = '" + tbStockDesc.Text.Replace( "'", "''" ) + "' WHERE stock_id = '" + strStockFlowID + "'";
                    dbCommand.ExecuteNonQuery();
        ...
        private void getSupplierList() {
            try {
                ...
                cbStockSupplier.Items.Clear();
                while( dbReader.Read() ) {
                    cbStockSupplier.Items.Add( new SupplierItem( dbReader[ 0 ].ToString(), dbReader[ 1 ].ToString() ) );
                }
                dbReader.Close();
            } catch( Exception ex ) {
