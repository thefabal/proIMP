        ...
        private void btnStockSave_Click( object sender, EventArgs e ) {
            if( cbStockSupplier.SelectedIndex != -1 ) {
                if( strStockFlowID == string.Empty ) {
                    ...
                    this.dbCommand.CommandText = "UPDATE stock SET stock_date = '" + dtpStockDate.Value.ToString( "yyyy-MM-dd" ) + "', stock_supplier = '" + ( (SupplierItem)cbStockSupplier.SelectedItem ).SupplierID + "', stock_desc = '" + tbStockDesc.Text.Replace( "'", "''" ) + "' WHERE stock_id = '" + strStockFlowID + "'";
                    this.dbCommand.ExecuteNonQuery();
